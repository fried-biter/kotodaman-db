...
function createCharacterRowHtml(char) {
    ...
    if (char.gimmicks && char.gimmicks.length > 0) {
        gimmickHtml = char.gimmicks.map((g, index) => `<a href="#" class="js-quick-filter" data-name="tx_gimmick[]" data-value="${char.gim_t[index] || g}"><span class="badge-gimmick">${g}</span></a>`).join('');
    }
