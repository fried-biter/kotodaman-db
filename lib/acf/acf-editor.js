...
(function ($) {
    $(document).ready(function () {
        ...
        var $specJsonText = $('#koto-spec-json-text');
        if ($specJsonText.length && window.FileReader) {
            $specJsonText.on('dragover dragenter', function (e) {
                e.preventDefault();
                e.stopPropagation();
                $(this).addClass('koto-spec-json-dragover');
            });

            $specJsonText.on('dragleave dragend drop', function (e) {
                $(this).removeClass('koto-spec-json-dragover');
            });

            $specJsonText.on('drop', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                reader.onload = function (event) {
                    $specJsonText.val(event.target.result).trigger('input').focus();
                };
...
acf.addAction('new_field/name=multi_cond_type', function (field) {
    field.on('change', function () {
        ...
        const $repeater = $('.acf-field[data-name="' + repeaterFieldName + '"]');
